...
public class CreateNewOrderView extends VerticalLayout {
    ...
    /** Порядок выбора заказов в сетке доставок (для маршрута: 1 → 2 → 3). */
    private final List<OrderShortDto> selectedDeliveriesOrder = new ArrayList<>();
    /** Стоимость сложного маршрута по выбранным заказам (если выбрано 2+ и нажали «Рассчитать»). */
...
//        setsSearchField.setValueChangeTimeout(500); // Обновление через 500мс после остановки ввода
        ...
        if (deliveriesLayout != null) {
            cartColumn.add(deliveriesLayout);
        }
    ...
    private Div buildDeliveriesLayout() {
        ...
        deliveriesGrid.addComponentColumn(order -> {
            ...
            String cityLabel = currentCity == MultiCityViewService.City.PARNAS ? "Парнас" : "Ухта";
            ...
            tgBtn.addClickListener(e -> {
                ...
                try {
                    CascadeNotificationResult result = cascadeNotificationService.notifyExistingOrder(order, cityLabel);
                    if (result.telegramSent()) {
                        multiCityOrderService.updateOrderTelegramNotified(getOrderCity(), order.getId());
                    }
                    if (result.vkSent()) {
                        multiCityOrderService.updateOrderVkNotified(getOrderCity(), order.getId());
                    }
                    if (!result.anySent()) {
            ...
            yandexBtn.addClickListener(e -> {
                ...
                try {
                    ...
                    multiCityOrderService.updateOrderYandexClaimId(getOrderCity(), order.getId(), claimId);
        ...
        complexRouteBtn.addClickListener(e -> {
            ...
            try {
                ...
                multiCityOrderService.updateOrdersYandexClaimId(getOrderCity(), orderIds, claimId);
    ...
    private void refreshDeliveriesGrid() {
        ...
        List<OrderShortDto> list = multiCityOrderService.getDeliveryOrdersWithItemsBetweenDates(getOrderCity(), from, to);
        list = list.stream()
